...
public class SummerAle extends Composition {
  ...
  public SummerAle() {
    ...
    patcher1.add(sine1);
    patcher1.add(new Gain(.076));

    Patch patcher2 = new Patch();
    sine2 = new Sine();
    patcher2.add(sine2);
    patcher2.add(new Gain(.09));

    Patch patcher3 = new Patch();
    sine3 = new Sine();
    patcher3.add(sine3);
    patcher3.add(new Gain(.09));

    Patch patcher4 = new Patch();
    sine4 = new Sine();
    patcher4.add(sine4);
    patcher4.add(new Gain(.09));
    ...
    if (cur >= 0 && (Math.abs(cur - start) >= len) && (cur != start || cur == 0)) {
      ...
      if (true) {
        ...
        if (noteChange < .9) {
          ...
          sine4.setFreq((deviation + 1.) * base * getRoot());
        }
        if (noteChange < .02) {
  ...
  public void next(double offset, double[] inputVal, double sampleRate) {
    super.next(Math.random() < .97 ? 1 : (Math.random() < .5 ? 0 : 2), inputVal, sampleRate);
  }
}
