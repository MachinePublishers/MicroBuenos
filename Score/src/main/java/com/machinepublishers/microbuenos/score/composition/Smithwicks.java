...
import com.machinepublishers.microbuenos.score.core.gen.Square;
...
public class Smithwicks extends Composition {

  private static Patch master = new Patch();
  private final Tone sine1;
  private final Tone sine2;
  private final Tone sine3;
  double[] loFreqs = new double[]{Note.TwelveTET.instance().getFreq(0, 1),
      Note.TwelveTET.instance().getFreq(3, 2),
  ...
  double[] hiFreqs2 = new double[]{Note.TwelveTET.instance().getFreq(3, 8),
      ...
      Note.TwelveTET.instance().getFreq(5, 2),
      Note.TwelveTET.instance().getFreq(7, 2),
      Note.TwelveTET.instance().getFreq(8, 2),
  ...
  public void next(double offset, double[] inputVal, double sampleRate) {
    super.next(Math.random() < .99 ? 1 : (Math.random() < .5 ? 0 : 2), inputVal, sampleRate);
  }
}
